...
interface NtVerseNavigatorProps {
  currentRef: string;
...
export function NtVerseNavigator({ currentRef, onSelect }: NtVerseNavigatorProps) {
  ...
  const maxChapters = getNtBook(pickerBook)?.chapters ?? 1;
  const maxVerse = getNtMaxVerse(pickerBook, pickerChapter);

  const prevRef = getPrevNtReference(activeBook, activeChapter, activeVerse);
  const nextRef = getNextNtReference(activeBook, activeChapter, activeVerse);
  ...
  const goToRef = () => {
    commitRef(normalizeNtReference(quickRef));
  };

  const castLots = () => {
    setCastingLots(true);
    window.setTimeout(() => {
      commitRef(getRandomKoineVerseRef());
  ...
  const pickerItemClass = (selected: boolean) =>
    selected
      ? 'bg-[var(--pw-accent)] text-[var(--pw-bg-app)] font-medium'
    ...
    <div className="w-full space-y-2">
      <div className="flex items-center gap-2">
        ...
        <button
          ...
          aria-label="Open New Testament selection"
        >
          <span className="font-mono tracking-tight">{currentRef}</span>
          <span className="text-[var(--pw-accent)] text-[10px] select-none">
            {isExpanded ? '−' : '+'}
          </span>
        </button>
      ...
      {isExpanded && (
        <div className="p-3 bg-[var(--pw-bg-surface)] border border-[var(--pw-border)] rounded-md text-xs space-y-3">
          ...
          <button
            ...
            title="Cast lots among verses with Greek interlinear loaded"
          >
            {castingLots ? 'Casting lots…' : 'Cast lots (Greek-loaded verses)'}
          </button>
        </div>
      )}
    </div>
